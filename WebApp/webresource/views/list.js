...
function (TuanApp, c, TuanBaseView, CommonPageFactory, WidgetGuider, MemCache, Util, cGeoService, WidgetFactory, Toast, TuanStore, TuanModels, TuanFilters, StoreManage, LazyLoad, ScrollObserver, listProduct, listBusiness) {
    ...
    View = PageView.extend({
        ...
        * @returns {string}
        */
        fallbackAddress: function (gps) {
            return (gps.city || searchStore.getAttr('ctyName') || '') + MSG.cityCenter;
        },
        ...
        isFromDetail: function (refer) {
            return refer && refer.match(/detail|listmap/i);
        },
        ...
        */
        ...
        onShow: function (refer) {
            refer = refer || this.getLastViewName();
            this.referUrl = refer;
            //定位提示容
            ...
            //如果从详情页后退不请求数据
            if (!this.isFromDetail(refer) || !MemCache.getItem('hasListData')) {
                //显示loading
                this.showLoading();
                //清空列表
                this.listWrap.empty();
